// Visualizes Meta Quest environment depth (Reddit / Depth API tutorial style).
// Samples global _PreprocessedEnvironmentDepthTexture (Texture2DArray) set by EnvironmentDepthManager.
Shader "QuestObjectron/EnvironmentDepthVisualization"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                fixed4 depthSample = SampleDepth(uv, eye);
                float energy = depthSample.r + depthSample.g + depthSample.b + depthSample.a;

                // Magenta = depth texture not bound yet.
                if (energy < 0.0001)
                {
                    return fixed4(1, 0, 1, 1);
                }

                // Preprocessed texture is often usable as RGBA; also support R-only heatmap.
